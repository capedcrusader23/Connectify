...
res.render('ans',{ans:ans,sign:req.user})
});
...
var que=new ques()
que.ques=req.body.question
que.ask=req.user.name
que.qid=Math.floor(Math.random()*10000000)
que.askedby=req.user.Eid
  que.upvotes=[]
  que.downvote=[]
  if(req.body.Education=='on')
  {
    que.category.push("Education")
  }
  if(req.body.Sports=='on')
  {
    que.category.push("Sports")
  }
  if(req.body.Science=='on')
  {
      que.category.push("Science")
  }
  if(req.body.Politics=='on')
  {
        que.category.push("Politics")
  }
  if(req.body.Humor=='on')
  {
        que.category.push("Humor")
  }
que.time=new Date();
que.save().then(function(){
  res.redirect('/user');
});
